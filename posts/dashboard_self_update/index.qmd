...
The idea of having something like a live server on a static website somehow fascinates me! So I tried to use `rvest` to scrape a german website which hosts relative up-to-date discharge values of rivers in Baden-Würrtemberg. Sadly I was not able to read the table (yet!), but I tried something else.
...
A Quarto dashboard is a website consisting of one or more pages, where you can visualize your data, do some monitoring and also add interactivity (Shiny and ojs)! To create one yourself, you just have to create a new Quarto project with a `.qmd` file and change the format to *dashboard*. 
...
# Use GitHub-actions and -pages to publish our dashboard 
